(() => {
  ...
  async function rpc(name, body={}, keepalive=false){
    const response = await fetch(`${SUPABASE_URL}/rest/v1/rpc/${name}`, {
      ...
      body:JSON.stringify(body), cache:'no-store', keepalive
    });
  ...
  function ensurePanel(){
    ...
    const volume=document.getElementById('quickVolume'); if(volume?.parentNode) volume.parentNode.insertBefore(panel,volume); else document.body.appendChild(panel);
    return panel;
  }

  function render(data){
    if(!data || typeof data!=='object') return; ensurePanel();
    const map={visitors:data.visitors ?? data.total_visitors,games:data.games ?? data.total_games,online:data.online ?? data.total_online};
    for(const [key,value] of Object.entries(map)){ const n=document.querySelector(`[data-s2="${key}"]`); const v=Number(value); if(n&&Number.isFinite(v)) n.textContent=v.toLocaleString('fr-FR'); }
  }

  async function heartbeat(){
    if(heartbeatBusy || document.visibilityState==='hidden' || !navigator.onLine) return;
    heartbeatBusy=true;
    try{ render(await rpc('snake2_heartbeat',{p_session_id:sessionId,p_visitor_id:visitorId})); }
    catch(e){ console.debug('[Snake2 stats heartbeat]',e.message); }
    finally{ heartbeatBusy=false; }
  }

  let lastCompletedAt=0;
  async function trackCompletedLevel(){
    const now=Date.now(); if(now-lastCompletedAt<1500) return; lastCompletedAt=now;
    try{ render(await rpc('snake2_game_started')); } catch(e){ console.debug('[Snake2 stats level]',e.message); }
  }
  ...
  function startRealtime(){
    ...
    realtimeChannel=realtimeClient.channel('snake2-public-stats')
      .on('postgres_changes',{event:'UPDATE',schema:'public',table:'snake2_stats',filter:'id=eq.1'},payload=>render(payload.new||{}))
      .subscribe(status=>{ if(status==='SUBSCRIBED') heartbeat(); });
  }

  function leave(){
    try{ realtimeClient?.removeChannel(realtimeChannel); }catch(_){}
    rpc('snake2_leave',{p_session_id:sessionId,p_visitor_id:visitorId},true).catch(()=>{});
  }
  ...
  async function boot(){
    ensurePanel();
    try{ render(await rpc('snake2_register_visitor',{p_visitor_id:visitorId})); }catch(e){ console.debug('[Snake2 stats visitor]',e.message); }
    await heartbeat();
    startRealtime();
